...
class _Filter_ScreenState extends State<Filter_Screen> {
                                  ...
                                  Navigator.push(
                                    ...
                                    if (value != null) {
                                      if (!value.isEmpty) {
                                        ...
                                        get_cityystate(sharedPreferences, pref.getString("filter_statename")!);
                                      } else {
                                        setState(() {
                                          stateValue = "";
                                          should_load = true;
                                ...
                                Navigator.push(
                                  ...
                                  if (value != null) {
                                    if (!value.isEmpty) {
                                      ...
                                      setState(() {
                                        if (pref.getString("filter_resource") != null) {
  ...
  Future<List<State_data>?> get_statecity(SharedPreferences sharedPreferences, int i, String countryname) async {
    ...
    if (isOnline) {
      ...
      Fluttertoast.showToast(
          msg: "Internet is required to fetch data..!!",
          toastLength: Toast.LENGTH_SHORT,
          gravity: ToastGravity.BOTTOM,
  ...
  Future<void> get_cityystate(SharedPreferences sharedPreferences, String state_value) async {

    bool isOnline = await globals.hasNetwork();
    if (isOnline) {
      showLoaderDialog(context);

      var requestUrl = "$base_url_listing/getCitiesbyState?state=$state_value";
      final uri_val = await http.get(
        Uri.parse(requestUrl),
        headers: <String, String>{"Content-Type": "application/json"},
      );

      print("city_data_3   $requestUrl");

      if (uri_val.statusCode == 200) {
        dynamic asdd = convert.jsonDecode(uri_val.body);
        if (asdd.containsKey('success') == true) {

          if (sharedPreferences.containsKey("json_city_by_state")) {
            sharedPreferences.remove("json_city_by_state");
          }
          sharedPreferences.setString("json_city_by_state", uri_val.body);
          if (sharedPreferences.containsKey("json_city_by_state")) {
            Navigator.pop(context, sharedPreferences.containsKey("filter_statename") ? sharedPreferences.getString("filter_statename") : "");
          }
        }
      }
    }
